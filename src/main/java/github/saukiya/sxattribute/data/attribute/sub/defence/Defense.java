...
public class Defense extends SubAttribute {
    ...
    public double calculationCombatPower(double[] values) {
        double value = (values[0] + values[1]) / 2 * getConfig().getInt("Defense.CombatPower");
        value += (values[2] + values[3]) / 2 * getConfig().getInt("PVPDefense.CombatPower");
        value += (values[4] + values[5]) / 2 * getConfig().getInt("PVEDefense.CombatPower");
        return value;
    }
}
