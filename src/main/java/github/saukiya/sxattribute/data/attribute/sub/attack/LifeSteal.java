...
public class LifeSteal extends SubAttribute {
    ...
    public void correct(double[] values) {
        ...
        values[0] = Math.min(values[0], getConfig().getInt("LifeStealRate.UpperLimit", 100));
        values[1] = Math.min(values[0], getConfig().getInt("LifeSteal.UpperLimit", 100));
    }

    @Override
    public double calculationCombatPower(double[] values) {
        return values[0] * getConfig().getInt("LifeStealRate.CombatPower") +
                values[1] * getConfig().getInt("LifeSteal.CombatPower");
    }
}
