...
插件同时提供来源生命周期、套装与词缀、强化与升星、重铸与洗练、宝石孔、附魔成长等独立功能。每个功能都有独立配置和开关，关闭后保留物品数据而不再产生效果。

## 开始使用

1. 下载与服务端版本匹配的 SX-Attribute，并将 SX-Attribute 与 [SX-Item](https://github.com/Saukiya/SX-Item/releases) 一同放入 `plugins/`。
